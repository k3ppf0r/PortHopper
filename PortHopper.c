      ...
      if(FD_ISSET(t[i],&check_list))
      {
        ...
        if(len>0 && send(t[i==0],buf,len,0)>0 )
        {
          ...
          if(ph_hex)
          {
            ...
            for(i=0;i<len;++i)
            {
              if(i%16==0)fprintf(ph_hex,"\n");
              fprintf(ph_hex,"%02X ",buf[i]);
            }
            fflush(ph_hex);
          }
        }
        else
        {
          ...
          if(ph_log)fprintf(ph_log,"\n[+]  Connection <Total %d> Cutdown, Total : %d Bytes\n\n",total_connect,total_byte),fflush(ph_log);
          break;
        }
      }
    }
...
int ph_slave(const char* ip1_str,unsigned short port1,const char* ip2_str,unsigned short port2)
{
...
int ph_listen(unsigned short port1,unsigned short port2)
{
  SOCKET s[2]={-1,-1};
  unsigned short p[2];
  p[0]=port1;
...
int ph_tran(unsigned short port1,const char* ip2_str,unsigned short port2)
{
...
#else
  fprintf(stdout,"\nBy:     k3ppf0r\n");
  fprintf(stdout,"About:  Port Data Transfer\n\n");
#endif
}
...
int main_func(int argc,char**argv)
{
  ...
  if (logpath)
  {
    setfile(&ph_log,logpath);
    if(ph_log==NULL)return 0;
  }
  if (hexpath)
  {
    setfile(&ph_hex,hexpath);
    if(ph_hex==NULL)return 0;
  }
  if (textpath)
  {
    setfile(&ph_text,textpath);
    if(ph_text==NULL)return 0;
  }
  switch(i)
  {
